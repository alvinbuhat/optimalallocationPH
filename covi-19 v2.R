...
Lat_t = HCF$Latitude[1:21]
Lon_t = HCF$Longitude[1:21]
...
e <- function(s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20){
  e=0
  for (i in 1:m){
    ...
    T[i,21] = (A-s1-s2-s3-s4-s5-s6-s7-s8-s9-s10-s11-s12-s13-s14-s15-s16-s17-s18-s19-s20)*fd[i,21]*I[i]/D[21]
...
ee <- function(s) e(s[1],s[2],s[3],s[4],s[5],s[6],s[7],s[8],s[9],s[10],s[11],s[12],s[13],s[14],s[15],s[16],s[17],s[18],s[19],s[20])
...
for (i in 1:m){
  for (j in 1:(n-1)){
    AA[i,j] = (fd[i,21]/D[21])-(fd[i,j]/D[j])
  }
  b[i] = A*fd[i,21]/D[21]-1
}
...
AA[(m+2*n),]=rep(1,20)
b[m+2*n]=A-M*L[21]
result <- constrOptim(rep(floor(90/n),n-1), ee, NULL, AA, b)
s=matrix(0, n,1)
s[1:(n-1),] = result$par
...
s_j = data.frame( testing_center = HCF$`Testing Center`[1:21], S_j = s)
print(s_j)
